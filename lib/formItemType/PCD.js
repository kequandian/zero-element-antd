...
export default function PCD(props) {
  ...
  function initData(pList) {
    const rst = [];
    findData(formdata[map[0].type], pList, rst, queryCityData).then(list => findData(formdata[map[1].type], list, rst, queryDistrictData)).then(list => findData(formdata[map[2].type], list, rst)).then(_ => {
