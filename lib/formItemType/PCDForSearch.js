...
export default function PCDForSearch(props) {
  const {
    className,
    options,
    namespace,
  ...
  const [dataSelectedOptions, setSelectedOptions] = useState([]); //保存选中数据

  function handleSetChangeData(value, selectedOptions) {
    setDataValue(value);
    setSelectedOptions(selectedOptions);
  } //关闭时触发 handleChange 函数


  function handlePopupVisibleChange(value) {
    if (!value) {
      handleChange(dataValue, dataSelectedOptions);
    }
  } //end
