...
export default function SelectFetch(props) {
  ...
  const [optionList, setOptionList] = useState([]); // 默认值

  const [defaultValue, setDefaultValue] = useState();
  const [isFindObj, setIsFindObj] = useState();
  const initRef = useRef(false);
  const effectFieldValue = formdata[effectField];
  const groupFieldValue = formdata[groupField]; //console.log(formdata,"formData")
  useWillMount(_ => {
  ...
  return /*#__PURE__*/React.createElement(Spin, {
    ...
    value: defaultValue
  }, pProps), optionList.map((opt, o) => /*#__PURE__*/React.createElement(Option, {
    key: opt[optValue] + o,
    value: opt[optValue]
  }, opt[optLabel]))));
}