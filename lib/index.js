"use strict";

var _interopRequireDefault = require("@babel/runtime/helpers/interopRequireDefault");

var _Load = _interopRequireDefault(require("./utils/Load"));

var _layout = require("zero-element/lib/config/layout");

var _container = require("zero-element/lib/config/container");

var _listAction = require("zero-element/lib/config/listAction");

var _formItemType = require("zero-element/lib/config/formItemType");

var _actionItemType = require("zero-element/lib/config/actionItemType");

var _valueType = require("zero-element/lib/config/valueType");

var _onRequest = _interopRequireDefault(require("./listAction/onRequest"));

// import { set as APIConfig } from 'zero-element/lib/config/APIConfig';
// APIConfig({
//   'DEFAULT_current': 1,
//   'DEFAULT_pageSize': 10,
//   'REQUEST_FIELD_current': 'pageNumber',
//   'REQUEST_FIELD_pageSize': 'pageSize',
//   'REQUEST_FIELD_field': 'field',
//   'REQUEST_FIELD_order': 'order',
//   'RESPONSE_FIELD_current': 'current',
//   'RESPONSE_FIELD_pageSize': 'size',
//   'RESPONSE_FIELD_total': 'total',
//   'RESPONSE_FIELD_records': 'records',
//   'RESPONSE_FIELD_PID': 'pid',
// });
(0, _layout.set)({
  Empty: (0, _Load["default"])('layout/Empty'),
  BaseTitle: (0, _Load["default"])('layout/BaseTitle'),
  TitleContent: (0, _Load["default"])('layout/TitleContent'),
  Loading: (0, _Load["default"])('layout/Loading'),
  Alone: (0, _Load["default"])('layout/Alone'),
  Row: (0, _Load["default"])('layout/Row'),
  SearchLayout: (0, _Load["default"])('layout/SearchLayout'),
  Grid: (0, _Load["default"])('layout/Grid'),
  Content: (0, _Load["default"])('layout/Content'),
  Items: (0, _Load["default"])('layout/Items')
});
(0, _container.set)({
  Empty: (0, _Load["default"])('container/Empty'),
  Table: (0, _Load["default"])('container/List/Table'),
  ReportList: (0, _Load["default"])('container/List/ReportList'),
  TreeTable: (0, _Load["default"])('container/List/TreeTable'),
  ChildrenList: (0, _Load["default"])('container/List/ChildrenList'),
  TreeList: (0, _Load["default"])('container/List/TreeList'),
  AutoReport: (0, _Load["default"])('container/List/AutoReport'),
  TableSelect: (0, _Load["default"])('container/List/TableSelect'),
  ItemList: (0, _Load["default"])('container/List/ItemList'),
  Search: (0, _Load["default"])('container/Form/Search'),
  AutoReportSearch: (0, _Load["default"])('container/Form/AutoReportSearch'),
  Form: (0, _Load["default"])('container/Form/Form'),
  ChildrenForm: (0, _Load["default"])('container/Form/ChildrenForm') // DnDFormEdit: Load('container/DnDFormEdit'),

});
(0, _listAction.set)({
  'onRequest': _onRequest["default"]
});
(0, _formItemType.set)({
  plain: (0, _Load["default"])('formItemType/Plain'),
  empty: (0, _Load["default"])('formItemType/Empty'),
  hidden: (0, _Load["default"])('formItemType/Hidden'),
  group: (0, _Load["default"])('formItemType/Group'),
  input: (0, _Load["default"])('formItemType/Input'),
  password: (0, _Load["default"])('formItemType/Password'),
  number: (0, _Load["default"])('formItemType/Number'),
  radio: (0, _Load["default"])('formItemType/Radio'),
  select: (0, _Load["default"])('formItemType/Select'),
  "switch": (0, _Load["default"])('formItemType/Switch'),
  checkbox: (0, _Load["default"])('formItemType/Checkbox'),
  map: (0, _Load["default"])('formItemType/Map'),
  pcd: (0, _Load["default"])('formItemType/PCD'),
  captcha: (0, _Load["default"])('formItemType/Captcha'),
  date: (0, _Load["default"])('formItemType/Date/date'),
  week: (0, _Load["default"])('formItemType/Date/week'),
  month: (0, _Load["default"])('formItemType/Date/month'),
  range: (0, _Load["default"])('formItemType/Date/range'),
  'table-select': (0, _Load["default"])('formItemType/TableSelect'),
  'modal-radio': (0, _Load["default"])('formItemType/ModalRadio'),
  'modal-checkbox': (0, _Load["default"])('formItemType/ModalCheckbox'),
  'upload-image': (0, _Load["default"])('formItemType/UploadImage'),
  'upload-file': (0, _Load["default"])('formItemType/UploadFile'),
  'checkbox-fetch': (0, _Load["default"])('formItemType/CheckboxFetch'),
  'select-fetch': (0, _Load["default"])('formItemType/SelectFetch'),
  'select-field': (0, _Load["default"])('formItemType/SelectField'),
  'text-area': (0, _Load["default"])('formItemType/TextArea'),
  'rich-text': (0, _Load["default"])('formItemType/RichText'),
  'one-mary': (0, _Load["default"])('formItemType/OneMary'),
  'number-range': (0, _Load["default"])('formItemType/NumberRange'),
  'pcdm': (0, _Load["default"])('formItemType/PCDM')
});
(0, _actionItemType.set)({
  modal: (0, _Load["default"])('actionItemType/Modal'),
  request: (0, _Load["default"])('actionItemType/Request'),
  'children-modal-add': (0, _Load["default"])('actionItemType/ChildrenModalAdd'),
  'import-excel': (0, _Load["default"])('actionItemType/ImportExcel'),
  'export-excel': (0, _Load["default"])('actionItemType/ExportExcel'),
  'table-checkbox': (0, _Load["default"])('actionItemType/TableCheckbox')
});
(0, _valueType.set)({
  'plain': (0, _Load["default"])('valueType/plain'),
  'status': (0, _Load["default"])('valueType/status'),
  'image': (0, _Load["default"])('valueType/image'),
  'tag': (0, _Load["default"])('valueType/tag'),
  'currency': (0, _Load["default"])('valueType/currency'),
  'percentage': (0, _Load["default"])('valueType/percentage'),
  'ellipsis': (0, _Load["default"])('valueType/ellipsis'),
  'input-number': (0, _Load["default"])('valueType/inputNumber'),
  'input-text': (0, _Load["default"])('valueType/inputText'),
  'input-select': (0, _Load["default"])('valueType/inputSelect'),
  'input-select-fetch': (0, _Load["default"])('valueType/inputSelectFetch')
});